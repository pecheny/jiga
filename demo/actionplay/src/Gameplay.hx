...
class Gameplay extends GameRunBase {
    public var F_BRD = new Vector2(-10, -10);
    public var L_BRD = new Vector2(10, 10);
    var spdval = 1.;
    @:once var input:GameInput<TGAxis, TGButts>;
    ...
    override function update(dt:Float) {
        if(!_inited)
            return;
        joe.spd.x = input.axes.getDirProjection(TGAxis.h);
        joe.pos += joe.spd * dt;
        // trace(joe.pos);
        for (i in 0...2)
            if (!inside(i))
